...
const UserManagement: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (parents.length === 0) fetchParents();
  }, []);
  ...
  const updateNewUser = (
    userId: string,
    field: keyof Omit<NewUser, "id" | "children">,
    value: string
  ): void => {
    setNewUsers(
      newUsers.map((user) =>
      ...
      newUsers.map((user) =>
        user.id === userId
          ? {
              ...
              children: [
                ...user.children,
                { id: generateUniqueCode("CHILD-"), name: "", grade: "" },
              ],
            }
          : user
      )
  ...
  const saveAllUsers = async () => {
    ...
    const usersToAdd = validUsers.map((user) => ({
      ...
      children: user.children
        .filter((child) => child.name.trim() && child.grade)
        .map((child) => ({
          ...child,
  ...
  const updateEditingUser = (
    field: keyof Omit<User, "id" | "children" | "addedDate">,
    value: string
  ): void => {
    if (editingUser) {
  ...
  const deleteUser = async (userId: string) => {
    await fetch(`/api/delete-parent`, {
      ...
      .then((res) => {
        ...
        setParents(
          parents.filter((user: { id: string }) => user.id !== userId)
        );
      })
    ...
    return (
      <div className="space-y-6">
        <div className="flex justify-between items-center">
          <div>
            <h2 className="text-2xl font-bold text-gray-900">Manage Parents</h2>
          ...
          <div className="bg-white rounded-lg shadow-md border border-gray-200">
            ...
            <div className="p-6 space-y-6">
              {newUsers.map((user, userIndex) => (
                <div
                  key={user.id}
                  className="border-l-4 border-l-blue-500 bg-white rounded-lg shadow border border-gray-200"
                >
                  <div className="p-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                      <div className="space-y-2">
                        ...
                        <input
                          ...
                          onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                            updateNewUser(user.id, "name", e.target.value)
                          }
                      ...
                      <div className="space-y-2">
                        ...
                        <input
                          ...
                          value={user.phone}
                          onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                            updateNewUser(user.id, "phone", e.target.value)
                          }
                        ...
                        >
                          ...
                          <div className="flex items-end">
                            {user.children.length > 1 && (
                              <button
                                onClick={() =>
                                  removeChildFromNewUser(user.id, child.id)
                                }
                        ...
                        >
                          ...
                          <div className="flex items-end">
                            {editingUser.children.length > 1 && (
                              <button
                                onClick={() =>
                                  removeChildFromEditingUser(child.id)
                                }
                                className="text-red-600 hover:text-red-700 border border-gray-300 hover:border-red-300 px-2 py-1 rounded text-sm transition-colors flex items-center w-full justify-center"
                              >
                                <Trash2 className="w-4 h-4 mr-1" />
